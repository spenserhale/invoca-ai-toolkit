...
.. list-table::
  ...
  * - autoRun
    ...
    - Default `true`
      ...
      determines whether Invocaâ€™s web integration runs when the page loads.

      If you have a single page application and would like to control when Invocaâ€™s web integration runs, set autoRun to false and call `Invoca.PNAPI.run()` whenever you are ready.
