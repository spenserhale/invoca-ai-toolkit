...
<KEY> is the RingPool key, which is provided through the RingPool wizard UI on the platform and should be kept a secret. The parameters are explained in detail below in the â€œAPI Parametersâ€ section.

The parameter names should match those chosen in the RingPool wizard, or you can use generic parameter names â€œparam1â€, â€œparam2â€, etc. Here is the same API URL with the generic parameter names:
...
**Note:** To access the campaign RingPoolâ€™s API URL navigate to the Campaign Summaryâ€™s RingPool tab and click â€œget api urlâ€ under Actions.
...
The response can be in any of these formats: **xml**, **json** or **csv**. To choose the format, change the â€œ.xmlâ€ shown in the URL to â€œ.jsonâ€ or â€œ.csvâ€. Below are example responses for each format:
...
.. list-table::
  ...
  * - mobile_click_to_call=
    - [Optional] Option to return mobile enabled links that populate the smartphoneâ€™s dialer when clicked.
