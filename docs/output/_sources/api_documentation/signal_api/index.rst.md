...
Passed in â€œapplication/jsonâ€ format.
...
Used to find the call associated with the signal.
    ...
    **Optional**
    ...
    `calling_phone_number` ANI in E.164 format +country national_number; example: â€˜+18885551212â€™.
...
Used to create the fields of a signal.
    ...
    **Required**
    ...
    `name` The name describing the signal event. For reporting a sale happened on a call, â€œSaleâ€ is recommended.
    Other examples include â€œFree Trialâ€, â€œ2yr Subscriptionâ€, â€œCancellation.â€
    ...
    **Optional**
    ...
    Note: this ID only needs to be unique within the given transaction and **name**, so it can be as simple as â€œ1â€, â€œ2â€â€¦ or
    ...
    `value` True or false as to whether the signal was met or not. Defaults to true if not passed. Can be a string â€˜trueâ€™ or â€˜falseâ€™, or 1 (true) or 0 (false), Yes (true), or No (false). These values are not case sensitive.
...
For example, if you make two requests with the same **name** and **partner_unique_id**, the other params in the second request will update the original signalâ€™s fields
