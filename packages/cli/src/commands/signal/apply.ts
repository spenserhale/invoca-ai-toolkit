...
export const signalApplyCommand = buildCommand({
  docs: { brief: "Apply signals and/or custom data to a call (POST)" },
  parameters: {
    flags: {
      ...signalFlags,
      ...safetyFlags,
  ...
  async func(this: void, flags: Flags) {
    await runCommand(async () => {
      ...
      if (flags["dry-run"]) {
        await emit({ status: "dry_run", method: "POST", body }, flags);
        return;
      }

      const client = buildClient(flags);
      const result = await client.signal.apply(body);
